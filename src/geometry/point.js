...
export function createPoint ({ x, y }) {
  function withX (x) { return createPoint({ x, y }) }

  function withY (y) { return createPoint({ x, y }) }

  function add (other) { return createPoint({ x: x + other.x, y: y + other.y }) }
  ...
  function multiply (factor) { return createPoint({ x: x * factor, y: y * factor }) }

  function map (mapper) { return mapper(createPoint({ x, y })) }
